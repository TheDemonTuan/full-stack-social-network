...
const LeftSide = () => {
  return (
    <div className={`${styles["left-side"]} bg-white flex flex-col p-2 gap-3 text-black shadow-2xl border-r border-black`}>
      ...
      {LeftSideData.map((item, index) => (
        <div key={index} className="flex gap-3 items-center p-2 hover:bg-black/30 rounded-lg cursor-pointer">
          <item.icon className="rounded-full p-2 bg-black text-white h-9 w-9" />
